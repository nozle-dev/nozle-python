...
Choose a dedicated customer in `DEMO_CUSTOMER_ID` and a random `DEMO_LOGIN_TOKEN` of at least 32 characters. Use the HTTPS React demo below to sign in; the merchant server on loopback HTTP is its internal proxy target. This token is only the demo's merchant login credential; it is not a Nozle key. Bind to loopback by default. Set `HOST` only when running behind your development proxy or inside an isolated test container.
